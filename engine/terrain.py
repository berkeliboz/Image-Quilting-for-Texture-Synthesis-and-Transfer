...
class Terrain(Entity):
    ...
    def get_basis_texture_at(self, row, col):
        return self.terrain_id_map.get((row, col)).basis_texture
    ...
    def get_texture_at(self,row,col):
        return self.terrain_id_map[(row, col)].texture
    ...
    def generate_terrain(self, number_of_horizontal_patches, number_of_vertical_patches):
        ...
        for row in range(number_of_vertical_patches):
            for col in range(1,number_of_horizontal_patches):
                ...
                if row == 0:
                    result_texture = image_quilting.find_ssd(self.get_basis_texture_at(row, col - 1), None, self.random_patches, direction(False, True, False, False))
                else:
            ...
            result_texture = image_quilting.find_ssd(None, self.get_basis_texture_at(row, 0), self.random_patches, direction(True, False, False, False))
    ...
    def fix_terrain(self,number_of_horizontal_patches, number_of_vertical_patches, row_offset, col_offset):
        ...
        for row in range(row_offset, number_of_vertical_patches + row_offset):
            for col in range(col_offset, number_of_horizontal_patches + col_offset - 1):
                ...
                self.get_texture_at(row, col)[:, new_size_middle: new_size] = texture[:, new_size_middle : new_size_middle + new_size_middle]
                self.get_texture_at(row, col + 1)[:, 0: new_size_middle] = texture[:, new_size : new_size + new_size_middle]

        for row in range(number_of_vertical_patches - 1):
            for col in range(number_of_horizontal_patches):
                image = image_quilting.stitch_vertical_lossy(self.get_texture_at(row, col), self.get_texture_at(row + 1, col))
                new_size = int(image.shape[0] / 2)
                new_size_middle = int(new_size/2)

                self.get_texture_at(row, col)[new_size_middle: new_size,:] = image[new_size_middle : new_size_middle + new_size_middle, :]
